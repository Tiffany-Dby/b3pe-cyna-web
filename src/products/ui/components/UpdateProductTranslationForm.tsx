...
} from "@/products/schemas/UpdateProductTranslationSchema";
import { UpdatedDetail } from "@/products/types/Products";
...
type UpdateProductTranslationField = Field & {
  name: keyof UpdateProductTranslationData;
...
const UpdateProductTranslationForm = ({ onError }: Props) => {
  const {
    selectedTranslation,
  ...
  const { form, handleSubmit, isLoading, serverError } = useCustomForm<
    ...
    UpdatedDetail
  >({
    schema: UpdateProductTranslationSchema,
    apiUrl: API_ROUTES.PRODUCT_UPDATE_TRANSLATION,
    defaultValues: {
      id: selectedTranslation?.id ?? "",
      descriptionTitle: selectedTranslation?.descriptionTitle ?? "",
      descriptionText: selectedTranslation?.descriptionText ?? "",
      benefits: selectedTranslation?.benefits ?? [
        { title: "", description: "" },
      ],
      specifications: selectedTranslation?.specifications ?? [
        { criteria: "", description: "" },
      ],
      functionalities: selectedTranslation?.functionalities ?? [{ value: "" }],
    },
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        >
          {isLoading
            ? t("products:updateTranslation.submit.loading")
            : t("products:updateTranslation.submit.action")}
        </Button>
      </form>
    </Form>
  );
...
export default UpdateProductTranslationForm;
